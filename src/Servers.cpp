...
{
    // A light client checks proof of work itself, so on a chain that changed algorithms it needs to
    // know which one applies at which height -- it cannot infer that from the headers it is verifying.
