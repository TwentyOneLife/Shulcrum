...
{
    // The header layout switches once and never switches back, so the predicate "is v2" is monotonic
    // in height and a binary search finds the boundary in ~log2(chain height) reads.
...
void Storage::loadCheckHeadersInDB()
{
    ...
    const Tic t0;
    {
        ...
        if (num) {
            ...
            // set genesis hash
            p->genesisHash = BTC::HeaderPoWHashRev(hVec.front()); // a genesis block is always v1, but ask rather than assume
            ...
            for (uint32_t i = 0; i < num; ++i) {
                ...
                bytes = BTC::HeaderPoWHash(bytes); // replace the header in the vector with its hash because it will be needed below...
            }
        }
    }
    if (num) {
...
std::vector<QByteArray> Storage::merkleCacheHelperFunc(unsigned int start, unsigned int count, QString *err)
{
    ...
    for (auto & ba : vec)
        ba = BTC::HeaderPoWHash(ba); // the merkle tree of headers is built from block ids, which are not SHA256d on a v2 header
    return vec;
}
